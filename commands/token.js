...
module.exports.run = async (client, message, args, reply) => {
  ...
  if (userBots.findIndex(b => b.id === bot) < 0)
    return reply(
      "<:redTick:568885082321059865> The specified bot is not listed."
    );
  userBots = userBots.filter(b => b.mainOwner === message.author.id);
  if (userBots.length < 1) return reply("You don't have any bots listed.");
  if (userBots.findIndex(b => b.id === bot) < 0)
    return reply(
      "<:redTick:568885082321059865> You do not own the specified bot."
    );

  if (args[0].toLowerCase() === "show") {
    const botIndex = userBots.findIndex(b => b.id === bot);
    const theBot = userBots[botIndex];
    if (theBot.token === null)
      return reply(
        "<:redTick:568885082321059865> Please generate a token using this command's `generate` option."
      );
    message.author.send(
      `The token attached to this bot is \`${theBot.token}\`.`
    );
    reply("<:greenTick:568885198519926784> The token has been DMed to you.");
  } else if (args[0].toLowerCase() === "generate") {
    const botIndex = userBots.findIndex(b => b.id === bot);
    const theBot = userBots[botIndex];
    require("../models/bots.js").findOne(
      { id: theBot.id },
      async (err, bott) => {
        ...
        reply(
          "<:greenTick:568885198519926784> Token has been generated for this bot. Please use this command's `show` option to view it."
        );
      }
    );
    ...
    require("../models/bots.js").findOne(
      ...
      async (err, bott) => {
        ...
        await bott.save().catch(e => console.log(e));
        reply(
          "<:greenTick:568885198519926784> Token has been regenerated for this bot. Please use this command's `show` option to view it."
        );
      }
    );
  } else {
